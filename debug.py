...
def extract_data(env, model, obs):
    ...
    while not done:
        ...
        # data["closest_distance"].append(np.linalg.norm(closest_point))
...
def main():
    ...
    with robotiqGymEnv(records=False, renders=False, savedir=saved_dir, store_data=True) as env:
        obs = env.reset()
        # steps = range(500)  # Define the number of steps here
        success = extract_data(env, model, obs)
    ...
    # plot_data([data["closest_distance"]], ["closest_distance"])
